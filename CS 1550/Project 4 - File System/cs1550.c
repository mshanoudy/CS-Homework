...
static int cs1550_mkdir(const char *path, mode_t mode)
{
    ...
    if (sizeof(path) > 10)
        return -EPERM;
    if (get_directory_index(directory) != -1)
        return -EEXIST;
    ...
    if (file == NULL)
    {
        perror("Error opening file in cs1550_mkdir");
        return -1;
    }
